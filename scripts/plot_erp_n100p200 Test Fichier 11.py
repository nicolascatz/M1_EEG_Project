...
# =================================================================
CSV_PATH = "BrainFlow-RAW_2026-09-08_10-47-23_11.csv"
SEP = "\t"                  # les fichiers bruts BrainFlow sont tabulés, pas séparés par des virgules
HAS_HEADER = False          # True si le fichier a une ligne de noms de colonnes

FS_NOMINAL = 250            # Hz, valeur de secours si l'auto-détection échoue
AUTO_FS = True              # si True, la fréquence réelle est estimée à partir de TIMESTAMP_COL
TIMESTAMP_COL = 22          # colonne Timestamp (format BrainFlow Cyton par défaut)

# Colonnes (index 0-based). Format d'export BrainFlow par défaut pour le Cyton :
# 0=package_num, 1-8=EEG, 9-11=Accel, 12-18=Other (le TTL tombe ici, colonne 14
# en comptage à partir de 1), 19-21=Analog, 22=Timestamp, 23=Marker.
TTL_COL = 13
EEG_COLS = [1, 2, 3, 4, 5, 6, 7, 8]   # EEG channel 1..8 (numérotation BrainFlow)
CHANNEL_NAMES = [f"EEG{i}" for i in range(1, len(EEG_COLS) + 1)]
CHANNEL_TO_PLOT = 0          # index dans EEG_COLS/CHANNEL_NAMES à afficher en détail
                             # (à ajuster selon l'électrode qui vous intéresse, ex. Cz)
...
# FONCTIONS
# =================================================================
def load_raw(csv_path, has_header, sep=","):
        ...
        with open(csv_path, "r") as f:
            for line in f:
                if line.startswith("%"):
                    ...
                    break
    df = pd.read_csv(csv_path, sep=sep, header=0 if has_header else None, skiprows=skiprows)
    df = df.dropna(how="any").reset_index(drop=True)  # écarte une éventuelle ligne finale incomplète
...
def estimate_fs(raw, timestamp_col, fallback_fs):
    """Estime la fréquence d'échantillonnage réelle à partir de la colonne Timestamp
    (les horloges des cartes réelles dérivent légèrement du nominal)."""
    try:
        ts = raw.iloc[:, timestamp_col].to_numpy(dtype=float)
        duration = ts[-1] - ts[0]
        fs = (len(ts) - 1) / duration
        if not np.isfinite(fs) or fs <= 0:
            raise ValueError
        return fs
    except Exception:
        print(f"Avertissement : échec de l'estimation de fs, valeur nominale {fallback_fs} Hz utilisée.")
        return fallback_fs
...
# TRAITEMENT
# =================================================================
def process_channel(eeg_continuous, onsets, fs):
...
def main():
    raw = load_raw(CSV_PATH, HAS_HEADER, sep=SEP)

    fs = estimate_fs(raw, TIMESTAMP_COL, FS_NOMINAL) if AUTO_FS else FS_NOMINAL
    print(f"Fréquence d'échantillonnage utilisée : {fs:.3f} Hz")
    ...
    for name, col in zip(CHANNEL_NAMES, EEG_COLS):
        ...
        results[name] = process_channel(eeg_continuous, onsets, fs)
    ...
    # annotation automatique des pics N100 / P200 (décalage en points, pas en
    # unités de données, pour ne jamais se chevaucher même si les deux pics
    # sont temporellement proches ou si l'un des deux n'est pas franchement positif)
    t_n1, a_n1 = find_peak(times, grand_avg, 0.06, 0.15, mode="min")
    t_p2, a_p2 = find_peak(times, grand_avg, 0.14, 0.26, mode="max")

    label_p2 = "P200" if a_p2 > 0 else "min. secondaire\n(pas de rebond positif net)"

    ax.plot(t_n1 * 1000, a_n1, "o", color="#C0392B", markersize=6, zorder=5)
    ax.annotate(f"N100\n{t_n1*1000:.0f} ms, {a_n1:.1f} µV",
                xy=(t_n1 * 1000, a_n1), xytext=(-45, -30), textcoords="offset points",
                fontsize=9, color="#C0392B",
                arrowprops=dict(arrowstyle="-", color="#C0392B", linewidth=0.8))
    ax.plot(t_p2 * 1000, a_p2, "o", color="#B7791F", markersize=6, zorder=5)
    ax.annotate(f"{label_p2}\n{t_p2*1000:.0f} ms, {a_p2:.1f} µV",
                xy=(t_p2 * 1000, a_p2), xytext=(35, 30), textcoords="offset points",
                fontsize=9, color="#B7791F",
                arrowprops=dict(arrowstyle="-", color="#B7791F", linewidth=0.8))
